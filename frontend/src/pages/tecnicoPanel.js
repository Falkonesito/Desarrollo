...
export default function TecnicoPanel() {
  ...
  const cargarAsignadas = async () => {
    try {
      setCargando(true);
      setError('');
      // ← ruta correcta (alias /mias también funcionará gracias al backend)
  ...
  const actualizarEstado = async (sol, nuevoEstado) => {
    ...
    try {
      ...
      setSolicitudes(prev => prev.map(s => s.id === sol.id ? { ...s, ...res.solicitud } : s));
    } catch (e) {
  ...
  const solicitudesFiltradas = useMemo(() => {
    ...
    return solicitudes.filter((s) => (s.estado_actual || 'pendiente') === filtroEstado);
      ...
      <header className="tecnico-header">
        <div className="header-content">
          <div className="header-left">
            ...
            <h1><i className="fas fa-user-cog"></i> Panel Técnico - INFOSER</h1>
          </div>
          <div className="header-info">
            <span className="user-info"><i className="fas fa-user me-2"></i>{user?.nombre || user?.email}</span>
      ...
      <div className="tecnico-layout">
        <nav className="tecnico-sidebar">
          <div className="perfil-card">
            <div className="tecnico-avatar"><i className="fas fa-user-cog"></i></div>
            ...
            <div className="estadisticas">
              <div className="stat-item"><div className="stat-number">{asignadas}</div><div className="stat-label">Asignadas</div></div>
              <div className="stat-item"><div className="stat-number">{enProceso}</div><div className="stat-label">En Proceso</div></div>
              <div className="stat-item"><div className="stat-number">{completadas}</div><div className="stat-label">Completadas</div></div>
            </div>
          </div>
          ...
          <div className="filtros-card">
            ...
            <div className="filtros-list">
              {['todas', 'pendiente', 'en_proceso', 'completada', 'cancelada'].map(op => (
                <button key={op}
          ...
          <div className="content-header">
            ...
            <div className="header-actions">
              <button className="btn-primary" onClick={cargarAsignadas} disabled={cargando}>
                <i className="fas fa-sync-alt me-2"></i>{cargando ? 'Actualizando...' : 'Actualizar'}
              </button>
            </div>
          </div>
                ...
                <div key={s.id} className="solicitud-card">
                  <div className="solicitud-header">
                    ...
                    <div className="solicitud-prioridad">
                      <span className={`prioridad-badge ${s.prioridad || 'media'}`}>{s.prioridad || 'media'}</span>
                    </div>
                  </div>
                  ...
                  <div className="solicitud-body">
                    ...
                    <div className="cliente-info">
                      <div className="info-item"><i className="fas fa-user"></i><strong>{s.cliente_nombre || '-'}</strong></div>
                      <div className="info-item"><i className="fas fa-envelope"></i>{s.cliente_email || ''}</div>
                      <div className="info-item"><i className="fas fa-map-marker-alt"></i>{s.direccion_servicio || ''}{s.comuna ? `, ${s.comuna}` : ''}{s.region ? `, ${s.region}` : ''}</div>
                    </div>

                    <p className="solicitud-descripcion">{s.descripcion}</p>

                    <div className="solicitud-meta">
                      <div className="meta-item"><i className="fas fa-calendar"></i>
                        <span>Solicitada: {fmtFecha(s.fecha_solicitud)}{s.fecha_actualizacion ? ` • Actualizada: ${fmtFecha(s.fecha_actualizacion)}` : ''}</span>
                      </div>
                      <div className="meta-item"><i className="fas fa-tools"></i><span>{s.tipo_servicio}</span></div>
                    </div>
